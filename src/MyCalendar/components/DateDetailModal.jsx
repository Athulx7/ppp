...
function DateDetailModal({ isOpen, onClose, detailDate, openModal, isOvertimeApplicable = true }) {
    return (
            ...
            footer={
                <div className="flex gap-2">
                    <button onClick={() => { onClose(); openModal('leave') }}
                        className="px-3 py-2 bg-indigo-600 text-white rounded-md text-xs font-semibold flex items-center gap-1">
                        <Plus size={12} /> Leave
                    </button>
                    <button onClick={() => { onClose(); openModal('regularize') }}
                        className="px-3 py-2 bg-emerald-600 text-white rounded-md text-xs font-semibold flex items-center gap-1">
                        <AlarmClock size={12} /> Regularize
                    </button>
                    <button onClick={() => { onClose(); openModal('reminder') }}
                        className="px-3 py-2 bg-amber-500 text-white rounded-md text-xs font-semibold flex items-center gap-1">
                        <BellPlus size={12} /> Remind
                    </button>
                    <button onClick={onClose}
                        className="px-4 py-2 border border-gray-300 text-gray-700 rounded-md text-xs font-medium hover:bg-gray-50 ml-auto">
                        Close
                    </button>
                </div>
            }>
                ...
                <div className="space-y-4">
                    {detailDate.attRec && (
                        <div className="p-4 bg-gray-50 rounded-2xl">
                            ...
                            <div className={`grid ${isOvertimeApplicable ? 'grid-cols-3' : 'grid-cols-2'} gap-3 text-center`}>
                                <div>
                                    <p className="text-xs text-gray-400">Punch In</p>
                                    <p className="text-sm font-bold text-gray-900">{detailDate.attRec.punch_in_time || detailDate.attRec.punch_in || '—'}</p>
                                </div>
                                <div>
                                    <p className="text-xs text-gray-400">Punch Out</p>
                                    <p className="text-sm font-bold text-gray-900">{detailDate.attRec.punch_out_time || detailDate.attRec.punch_out || '—'}</p>
                                </div>
